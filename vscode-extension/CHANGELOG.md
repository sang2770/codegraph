...
- Pick the projects you want to see instead of typing their keys. **Select projects…** in the board's filter bar — also **CodeBrain: Select Jira Projects**, and a button in the view's title bar — lists every project your account can browse, searchable by key or name, and remembers the ones you tick. The projects you are filtered on show as chips you can drop one at a time, and a key typed by hand still works, so a project the list could not be read for is never lost.
